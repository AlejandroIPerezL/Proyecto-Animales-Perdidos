...
Nota sobre el historial de versiones (Commits):  El alto número de commits registrados en este repositorio (alrededor de 30) se debe a múltiples intentos por configurar y gestionar la subida del proyecto utilizando Git. Durante las pruebas de validación, al intentar descargar el repositorio y correr el proyecto en Qt Creator, el programa presentaba errores de compilación y no se ejecutaba debido a conflictos con los archivos de configuración locales y la carpeta build. Tras varios intentos por solucionar estos problemas de rutas mediante sincronizaciones de código, tomé la decisión de limpiar por completo el repositorio y dejar únicamente el código fuente esencial. Por esta razón, para garantizar que el programa compile y se ejecute a la primera sin problemas de compatibilidad, la entrega final se debe descargar directamente como un archivo comprimido (.zip) para luego abrir el archivo CMakeLists.txt en el entorno de desarrollo.
